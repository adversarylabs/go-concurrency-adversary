#!/usr/bin/env node
import { createRequire as __goConcurrencyCreateRequire } from 'node:module'; const require = __goConcurrencyCreateRequire(import.meta.url);
...
var import__, ADVERSARY_MODEL_PROTOCOL_VERSION, ADVERSARY_MODEL_ENDPOINT_ENV, ADVERSARY_MODEL_TOKEN_ENV, DEFAULT_MODEL_TIMEOUT_MS, MAX_MODEL_TIMEOUT_MS, DEFAULT_MAXIMUM_OUTPUT_TOKENS, MAX_MAXIMUM_OUTPUT_TOKENS, MAX_PROMPT_BYTES, MAX_INPUT_BYTES, MAX_SCHEMA_BYTES, MAX_RESPONSE_BYTES, ModelUnavailableError, ModelReviewError, BrokerReviewModel;
var init_model = __esm({
  "node_modules/@adversarylabs/sdk/dist/model.js"() {
    import__ = __toESM(require__(), 1);
    ADVERSARY_MODEL_PROTOCOL_VERSION = 1;
    ADVERSARY_MODEL_ENDPOINT_ENV = "ADVERSARY_MODEL_ENDPOINT";
    ADVERSARY_MODEL_TOKEN_ENV = "ADVERSARY_MODEL_TOKEN";
    DEFAULT_MODEL_TIMEOUT_MS = 12e4;
    MAX_MODEL_TIMEOUT_MS = 6e5;
    DEFAULT_MAXIMUM_OUTPUT_TOKENS = 8192;
    MAX_MAXIMUM_OUTPUT_TOKENS = 65536;
    MAX_PROMPT_BYTES = 256 << 10;
    MAX_INPUT_BYTES = 4 << 20;
    MAX_SCHEMA_BYTES = 512 << 10;
    ...
    ModelReviewError = class extends Error {
      code;
      retryable;
...
var ADVERSARY_REPO_INDEX_ENV, REPO_INDEX_SCHEMA_VERSION, RepoIndexUnavailableError, MemoryRepoIndex;
var init_repo_index = __esm({
  "node_modules/@adversarylabs/sdk/dist/repo-index.js"() {
    ADVERSARY_REPO_INDEX_ENV = "ADVERSARY_REPO_INDEX";
    REPO_INDEX_SCHEMA_VERSION = "v1";
    RepoIndexUnavailableError = class extends Error {
...
var DEFAULT_MAX_ROUNDS, MAX_MAX_ROUNDS, DEFAULT_MAX_TOOL_CALLS, MAX_MAX_TOOL_CALLS, DEFAULT_MAX_TOTAL_BYTES, MAX_MAX_TOTAL_BYTES, DEFAULT_MAX_BYTES_PER_READ, MAX_MAX_BYTES_PER_READ, DEFAULT_MAX_LINES_PER_READ, MAX_MAX_LINES_PER_READ, DEFAULT_DIRECTORY_PAGE_SIZE, MAX_DIRECTORY_PAGE_SIZE, MAX_PATTERNS, MAX_PATTERN_LENGTH, MAX_OPERATION_PATH_LENGTH, MAX_OPERATIONS_PER_ROUND, PLANNING_OUTPUT_TOKENS, DEFAULT_PLANNING_TIMEOUT_MS, defaultExcludedSegments, repositoryPlanSchema;
var init_repository_model = __esm({
  "node_modules/@adversarylabs/sdk/dist/repository-model.js"() {
    init_model();
    DEFAULT_MAX_ROUNDS = 6;
    MAX_MAX_ROUNDS = 12;
    DEFAULT_MAX_TOOL_CALLS = 24;
    MAX_MAX_TOOL_CALLS = 128;
    DEFAULT_MAX_TOTAL_BYTES = 256 << 10;
    MAX_MAX_TOTAL_BYTES = 2 << 20;
    DEFAULT_MAX_BYTES_PER_READ = 32 << 10;
    MAX_MAX_BYTES_PER_READ = 256 << 10;
    DEFAULT_MAX_LINES_PER_READ = 400;
    MAX_MAX_LINES_PER_READ = 4e3;
    DEFAULT_DIRECTORY_PAGE_SIZE = 200;
    MAX_DIRECTORY_PAGE_SIZE = 1e3;
    MAX_PATTERNS = 128;
    MAX_PATTERN_LENGTH = 512;
    MAX_OPERATION_PATH_LENGTH = 4096;
    MAX_OPERATIONS_PER_ROUND = 8;
    PLANNING_OUTPUT_TOKENS = 1500;
    DEFAULT_PLANNING_TIMEOUT_MS = 12e4;
    defaultExcludedSegments = /* @__PURE__ */ new Set([
      ".git",
      ".hg",
      ".svn",
      "node_modules",
...
var DEFAULT_IGNORE_DIRECTORIES;
var init_sources = __esm({
  "node_modules/@adversarylabs/sdk/dist/sources.js"() {
    DEFAULT_IGNORE_DIRECTORIES = Object.freeze([
      ".git",
      ".next",
      "build",
      "coverage",
...
var import_yaml, ADVERSARY_MANIFEST_MAX_BYTES;
var init_manifest = __esm({
  "node_modules/@adversarylabs/sdk/dist/manifest.js"() {
    import_yaml = __toESM(require_dist(), 1);
    ADVERSARY_MANIFEST_MAX_BYTES = 1 << 20;
  }
});
...
var import__2, DEFAULT_INPUT_PATH, DEFAULT_OUTPUT_PATH, ADVERSARY_RUN_PROTOCOL_VERSION, verboseValues, envelopeValidator, Severity, DEFAULT_SEVERITY_GUIDANCE, Confidence, DEFAULT_CONFIDENCE_THRESHOLDS, WORKTREE_HEAD_REF, RuleRegistry, ruleRegistry, log, Adversary, MAX_OPINION_CONCERN_LENGTH, OPINION_CONCERN_REWRITE_PROMPT, OPINION_CONCERN_REWRITE_SCHEMA, DEFAULT_CONCERN_REWRITE_BUDGET, semanticStopWords;
var init_dist = __esm({
  "node_modules/@adversarylabs/sdk/dist/index.js"() {
    import__2 = __toESM(require__(), 1);
    init_model();
    init_repo_index();
    init_repository_model();
    init_sources();
    init_model();
    init_repository_model();
    init_repo_index();
    init_sources();
    init_manifest();
    DEFAULT_INPUT_PATH = "/adversary/input.json";
    DEFAULT_OUTPUT_PATH = "/adversary/output.json";
    ADVERSARY_RUN_PROTOCOL_VERSION = 1;
    verboseValues = /* @__PURE__ */ new Set(["1", "true", "TRUE", "yes", "YES"]);
    Severity = {
      Info: "info",
      Low: "low",
      Medium: "medium",
    ...
    WORKTREE_HEAD_REF = "WORKTREE";
    RuleRegistry = class _RuleRegistry {
    ...
    MAX_OPINION_CONCERN_LENGTH = 100;
    OPINION_CONCERN_REWRITE_PROMPT = `Rewrite the input text into a short noun phrase suitable after the words "I would address".
...
- Prefer the primary engineering concern over command inventories or headlines`;
    OPINION_CONCERN_REWRITE_SCHEMA = {
      type: "object",
      additionalProperties: false,
      required: ["concern"],
      properties: {
    ...
    semanticStopWords = /* @__PURE__ */ new Set([
      "a",
      "an",
      "and",
      "are",
...
// src/async-listener.ts
async function analyzeAsyncListenerOwnership(discovery) {
  const current = await parseRelevantFiles(discovery.files, false);
  const previous = discovery.mode === "diff" ? await parseRelevantFiles(discovery.files, true) : [];
  try {
    ...
    for (const fact of currentFacts) {
      ...
      const anchor = fact.evidence.find(({ parsed, node }) => changedEvidence(discovery.mode, parsed.file, node));
      if (discovery.mode === "diff" && anchor === void 0) continue;
...
async function parseRelevantFiles(files, previous) {
  const parsed = [];
  for (const file of files) {
    if (file.path.endsWith("_test.go")) continue;
    if (previous && file.status === "added" && file.previous === void 0) continue;
...
function collectFacts(files, modulePath) {
  const helpers = files.flatMap(collectAsyncHelpers);
  const serverTypes = files.flatMap(collectHTTPServerTypes);
  const methods = files.flatMap((parsed) => descendants(parsed.root, "method_declaration").map((node) => ({ parsed, node })));
  const facts = [];
  for (const server of serverTypes) {
    if (server.parsed.packageName === "main") continue;
    const typeMethods = methods.filter(({ parsed, node }) => samePackage(parsed, server.parsed) && receiverType(node, parsed.file.current) === server.name);
    if (typeMethods.some(({ parsed, node }) => methodStopsField(node, parsed.file.current, server.handlerField))) continue;
    for (const { parsed, node: method } of typeMethods) {
      ...
      const listenerOrigins = listenerVariables(body2, parsed);
      if (listenerOrigins.length === 0) continue;
      for (const call of directCallableDescendants(body2, "call_expression")) {
        if (!nodeIsReachable(call, body2, parsed.file.current)) continue;
        const helper = resolveAsyncHelper(call, parsed, helpers, modulePath, body2);
        if (helper === void 0) continue;
        const args2 = call.childForFieldName("arguments")?.namedChildren ?? [];
        const listener = args2[helper.listenerParameterIndex];
        const serveMethod = args2[helper.callbackParameterIndex];
        const listenerName = listener?.type === "identifier" ? sourceText(listener, parsed.file.current) : void 0;
        const listenerOrigin = listenerName === void 0 ? void 0 : listenerOrigins.filter((origin) => origin.name === listenerName && origin.declaration.endIndex < call.startIndex && declarationScopeContainsUse(origin.declaration, call)).sort((left, right) => right.declaration.startIndex - left.declaration.startIndex)[0];
        if (listener?.type !== "identifier" || listenerOrigin === void 0 || bindingChangesBetween(body2, listenerName, listenerOrigin.declaration.endIndex, call, parsed.file.current) || serveMethod === void 0 || !isHandlerServeMethod(serveMethod, parsed.file.current, receiver, server.handlerField, body2)) continue;
        const evidence = [
...
function collectHTTPServerTypes(parsed) {
  const httpAlias = aliasForPath(parsed.importsByAlias, "net/http");
  if (httpAlias === void 0) return [];
...
function collectAsyncHelpers(parsed) {
  const netAlias = aliasForPath(parsed.importsByAlias, "net");
  if (netAlias === void 0) return [];
  const source = parsed.file.current;
  const helpers = [];
  for (const fn of descendants(parsed.root, "function_declaration")) {
    ...
    const callback = parameters.find((parameter) => normalize(parameter.type) === `func${netAlias}.Listenererror`);
    if (listener === void 0 || callback === void 0) continue;
    const listenerName = listener.name;
    const callbackName = callback.name;
    for (const goStatement of directCallableDescendants(body2, "go_statement")) {
      if (!nodeIsReachable(goStatement, body2, source)) continue;
      if (bindingChangesBetween(body2, listenerName, body2.startIndex, goStatement, source) || bindingChangesBetween(body2, callbackName, body2.startIndex, goStatement, source)) continue;
      ...
      const callbackCall = directCallableDescendants(literalBody, "call_expression").find((call) => {
        ...
        const args2 = call.childForFieldName("arguments")?.namedChildren ?? [];
        return called?.type === "identifier" && sourceText(called, source) === callbackName && !callableParameterNamed(literalBody, callbackName, source) && !localBindingShadowsAtUse(literalBody, callbackName, call, source) && !bindingChangesBetween(literalBody, callbackName, literalBody.startIndex, call, source) && args2.length === 1 && args2[0]?.type === "identifier" && sourceText(args2[0], source) === listenerName && !callableParameterNamed(literalBody, listenerName, source) && !bindingChangesBetween(literalBody, listenerName, literalBody.startIndex, args2[0], source) && !localBindingShadowsAtUse(literalBody, listenerName, args2[0], source);
      });
      if (callbackCall === void 0) continue;
      if (helperStopsListener(body2, source, listenerName, callbackCall)) continue;
      helpers.push({
        parsed,
        name: sourceText(name2, source),
...
function resolveAsyncHelper(call, caller, helpers, modulePath, callerBody) {
  const fn = call.childForFieldName("function");
  if (fn?.type === "identifier") {
    const name3 = sourceText(fn, caller.file.current);
    if (callableParameterNamed(callerBody, name3, caller.file.current) || localBindingShadowsAtUse(callerBody, name3, call, caller.file.current)) return void 0;
    return helpers.find((helper) => helper.name === name3 && samePackage(helper.parsed, caller));
  }
  ...
  if (callableParameterNamed(callerBody, alias, caller.file.current) || localBindingShadowsAtUse(callerBody, alias, call, caller.file.current)) return void 0;
  const importPath = caller.importsByAlias.get(alias);
  if (importPath === void 0) return void 0;
  const name2 = sourceText(field, caller.file.current);
  return helpers.find((helper) => helper.name === name2 && importResolvesTo(importPath, helper.parsed, modulePath));
}
function startContract(method, parsed) {
  const contextAlias = aliasForPath(parsed.importsByAlias, "context");
  if (contextAlias === void 0) return false;
  const source = parsed.file.current;
  const parameters = directParameters(method);
  const hasContext = parameters.some((parameter) => normalize(sourceText(parameter.childForFieldName("type") ?? parameter, source)) === `${contextAlias}.Context`);
  const result = method.childForFieldName("result");
  return hasContext && result !== null && /(^|[^A-Za-z0-9_])error([^A-Za-z0-9_]|$)/.test(sourceText(result, source));
}
function listenerVariables(body2, parsed) {
  const source = parsed.file.current;
  const netAlias = aliasForPath(parsed.importsByAlias, "net");
  if (netAlias === void 0) return [];
  const names = [];
  for (const assignment of directCallableDescendants(body2, "short_var_declaration")) {
    ...
    for (let index = 0; index < right.length; index += 1) {
      ...
      if (fn?.type === "selector_expression" && sourceText(fn.childForFieldName("operand") ?? fn, source) === netAlias && sourceText(fn.childForFieldName("field") ?? fn, source) === "Listen" && !callableParameterNamed(body2, netAlias, source) && !localBindingShadowsAtUse(body2, netAlias, value, source) && nodeIsReachable(value, body2, source)) {
...
function methodStopsField(method, source, fieldName) {
  const body2 = method.childForFieldName("body");
  const receiver = receiverName(method, source);
  if (body2 === null || receiver === void 0) return false;
  const aliases = /* @__PURE__ */ new Map();
  for (const declaration of directCallableDescendants(body2, "short_var_declaration")) {
    ...
    if (isReceiverField(right[0], source, receiver, fieldName, body2)) {
      aliases.set(sourceText(left[0], source), declaration);
    }
  }
  return descendants(body2, "call_expression").some((call) => {
    if (!callIsDefinitelyActive(call, body2) || !nodeIsReachable(call, body2, source)) return false;
    const fn = call.childForFieldName("function");
    ...
    if (isReceiverField(operand, source, receiver, fieldName, body2)) return true;
    if (operand.type !== "identifier") return false;
    const alias = sourceText(operand, source);
    const declaration = aliases.get(alias);
    return declaration !== void 0 && declaration.endIndex < call.startIndex && declarationScopeContainsUse(declaration, call) && !bindingChangesBetween(body2, alias, declaration.endIndex, call, source);
  });
}
function helperStopsListener(body2, source, listenerName, callbackCall) {
  return descendants(body2, "call_expression").some((call) => {
    if (!callIsDefinitelyActive(call, body2) || !nodeIsReachable(call, body2, source)) return false;
    if (!unconditionallyExecutedWithinCallable(call, body2, source)) return false;
    const fn = call.childForFieldName("function");
    if (fn?.type !== "selector_expression") return false;
    const operand = fn.childForFieldName("operand");
    const field = fn.childForFieldName("field");
    if (operand?.type !== "identifier" || field === null || sourceText(operand, source) !== listenerName || localBindingShadowsAtUse(body2, listenerName, operand, source) || nestedCallableParameterShadowsAtUse(body2, listenerName, operand, source)) return false;
    const selected = sourceText(field, source);
    if (!/^(?:Close|Shutdown|Stop)$/.test(selected)) return false;
    if (bindingChangesBetween(body2, listenerName, body2.startIndex, call, source)) return false;
...
function unconditionallyExecutedWithinCallable(node, callableBody, source) {
  let current = node;
  while (current !== null && current.id !== callableBody.id) {
    ...
    if (containingIndex >= 0 && statements.slice(0, containingIndex).some((statement) => statementCanBypass(statement, callableBody, source))) return false;
...
function statementCanBypass(statement, callableBody, source) {
  const candidates = [
    statement,
    ...directCallableDescendants(statement, "return_statement"),
  ...
  if (candidates.some((candidate) => ["return_statement", "break_statement", "continue_statement", "goto_statement"].includes(candidate.type) && nodeIsReachable(candidate, callableBody, source))) return true;
  return directCallableDescendants(statement, "call_expression").some((call) => {
    ...
    const fn = call.childForFieldName("function");
    return fn?.type === "identifier" && sourceText(fn, source) === "panic" && !callableParameterNamed(callableBody, "panic", source) && !localBindingShadowsAtUse(callableBody, "panic", call, source) && !nestedCallableParameterShadowsAtUse(callableBody, "panic", call, source) && !packageBindingNamed(callableBody, "panic", source);
  });
}
...
function isHandlerServeMethod(node, source, receiver, fieldName, body2) {
  ...
  return operand !== null && field !== null && sourceText(field, source) === "Serve" && isReceiverField(operand, source, receiver, fieldName, body2);
}
function isReceiverField(node, source, receiver, fieldName, body2) {
  if (node.type !== "selector_expression") return false;
  const operand = node.childForFieldName("operand");
  const field = node.childForFieldName("field");
  return operand?.type === "identifier" && field !== null && sourceText(operand, source) === receiver && sourceText(field, source) === fieldName && (body2 === void 0 || !localBindingShadowsAtUse(body2, receiver, operand, source) && !nestedCallableParameterShadowsAtUse(body2, receiver, operand, source) && !bindingChangesBetween(body2, receiver, body2.startIndex, operand, source));
}
function localBindingShadowsAtUse(body2, name2, use, source) {
  const declarations = [
    ...descendants(body2, "short_var_declaration"),
    ...descendants(body2, "var_spec"),
    ...descendants(body2, "const_spec"),
    ...descendants(body2, "range_clause").filter((clause) => sourceText(clause, source).includes(":="))
  ];
  return declarations.some((declaration) => declaration.startIndex < use.startIndex && bindingNames(declaration, source).includes(name2) && declarationScopeContainsUse(declaration, use));
}
function callableParameterNamed(body2, name2, source) {
  const callable = body2.parent;
  return callable !== null && callable.childForFieldName("body")?.id === body2.id && directParameters(callable).some((parameter) => declaredNames(parameter, source).includes(name2));
}
function nestedCallableParameterShadowsAtUse(body2, name2, use, source) {
  let current = use.parent;
  while (current !== null && current.id !== body2.id) {
    if (current.type === "func_literal" && directParameters(current).some((parameter) => declaredNames(parameter, source).includes(name2))) return true;
    current = current.parent;
  }
  return false;
}
function declarationScopeContainsUse(declaration, use) {
  ...
  while (current !== null && current.id !== block.id) {
    if ([
      ...
      "type_switch_statement",
      "select_statement"
    ].includes(current.type)) return containsNode(current, use);
    current = current.parent;
  }
  return containsNode(block, use);
}
function bindingChangesBetween(body2, name2, startIndex, use, source) {
  const changes = [
  ...
  return changes.some((change) => {
    if (change.startIndex <= startIndex || change.endIndex >= use.startIndex || !bindingNames(change, source).includes(name2) || !sameCallableScope(change, use, body2) && !directIIFEChangeCanAffectUse(change, use, body2, source) || !nodeIsReachable(change, body2, source)) return false;
    ...
    return !localBindingDeclaredAfter(body2, name2, startIndex, change, source) && !nestedCallableParameterShadowsAtUse(body2, name2, change, source);
  });
}
function directIIFEChangeCanAffectUse(change, use, body2, source) {
  const literal = nearestAncestor(change, "func_literal");
  if (literal === null) return false;
  const execution = directLiteralExecution(literal);
  return execution?.type === "expression_statement" && execution.endIndex < use.startIndex && sameCallableScope(execution, use, body2) && nodeIsReachable(execution, body2, source);
}
function localBindingDeclaredAfter(body2, name2, startIndex, use, source) {
  const declarations = [
    ...descendants(body2, "short_var_declaration"),
    ...descendants(body2, "var_spec"),
    ...descendants(body2, "const_spec"),
  ...
  return declarations.some((declaration) => declaration.startIndex > startIndex && declaration.startIndex < use.startIndex && bindingNames(declaration, source).includes(name2) && declarationScopeContainsUse(declaration, use));
}
function sameCallableScope(left, right, body2) {
  return callableScope(left, body2).id === callableScope(right, body2).id;
}
function callableScope(node, body2) {
...
function bindingNames(node, source) {
...
function nodeIsReachable(node, callableBody, source) {
  let current = node;
  while (current !== null && current.id !== callableBody.id) {
    const parent = current.parent;
    if (parent === null) return false;
    if (parent.type === "if_statement") {
      ...
      const alternative = parent.childForFieldName("alternative");
      const value = condition === null ? void 0 : staticBoolean(condition, source);
      if (value === false && consequence !== null && containsNode(consequence, current) || value === true && alternative !== null && containsNode(alternative, current)) return false;
    }
    const statements = directStatements(parent);
    const containingIndex = statements.findIndex((statement) => containsNode(statement, current));
    if (containingIndex >= 0 && statements.slice(0, containingIndex).some((statement) => unconditionallyTerminates(statement, node, callableBody, source))) return false;
    current = parent;
  }
  return current?.id === callableBody.id;
}
function controlFlowEvidence(node, callableBody) {
  const evidence = [];
  let current = node.parent;
  while (current !== null && current.id !== callableBody.id) {
...
function terminationOutcomes(statement, target, callableBody, source) {
  ...
  if (statement.type === "expression_switch_statement" || statement.type === "type_switch_statement") {
    ...
    const resolved = new Array(cases.length);
    for (let index = cases.length - 1; index >= 0; index -= 1) {
      const raw = blockTerminationOutcomes(cases[index], target, callableBody, source);
      resolved[index] = raw & ~flowFallsThrough | ((raw & flowFallsThrough) === 0 ? 0 : resolved[index + 1] ?? flowContinues);
    }
  ...
  if (statement.type === "for_statement") {
    ...
    const escapingBreak = body2 !== null && directCallableDescendants(statement, "break_statement").some((candidate) => nodeIsReachable(candidate, body2, source) && breakExitsLoop(candidate, statement, source));
    const escapingGoto = body2 !== null && directCallableDescendants(statement, "goto_statement").some((candidate) => nodeIsReachable(candidate, body2, source) && gotoExitsLoop(candidate, statement, source));
    return infinite && !escapingBreak && !escapingGoto ? flowTerminates : flowContinues;
  }
...
function breakExitsLoop(statement, loop, source) {
  ...
  if (labelNode !== void 0) {
    ...
    return descendants(root, "labeled_statement").some((candidate) => {
      const name2 = candidate.namedChildren[0];
      return name2 !== void 0 && sourceText(name2, source) === labelName && containsNode(candidate, loop);
    });
  }
  let current = statement.parent;
  while (current !== null) {
    if (["for_statement", "expression_switch_statement", "type_switch_statement", "select_statement"].includes(current.type)) return current.id === loop.id;
    current = current.parent;
  }
  return false;
}
function gotoExitsLoop(statement, loop, source) {
  const labelNode = statement.namedChildren[0];
  if (labelNode === void 0) return true;
  let root = loop;
  while (root.parent !== null) root = root.parent;
  const labelName = sourceText(labelNode, source);
  const label = descendants(root, "labeled_statement").find((candidate) => {
    const name2 = candidate.namedChildren[0];
    return name2 !== void 0 && sourceText(name2, source) === labelName;
  });
  return label === void 0 || !containsNode(loop, label);
}
...
function packageBindingNamed(callableBody, name2, source) {
  ...
  if (declarations.some((declaration) => !insideCallableDeclaration(declaration) && declaredNames(declaration, source).includes(name2))) return true;
  return descendants(root, "import_spec").some((spec) => {
    const text = sourceText(spec, source).trim();
    const match = /^(?:(\.|_|[A-Za-z_]\w*)\s+)?["`]([^"`]+)["`]$/.exec(text);
    return match !== null && (match[1] ?? match[2].split("/").at(-1)) === name2;
  });
}
function insideCallableDeclaration(node) {
  let current = node.parent;
  while (current !== null) {
    if (current.type === "function_declaration" || current.type === "method_declaration" || current.type === "func_literal") return true;
    current = current.parent;
  }
  return false;
}
...
function directCallableDescendants(node, type) {
  const result = [];
...
function directParameters(callable) {
  const parameters = callable.childForFieldName("parameters");
  return parameters === null ? [] : parameters.namedChildren.filter((node) => node.type === "parameter_declaration" || node.type === "variadic_parameter_declaration");
}
function directParameterBindings(callable, source) {
  const result = [];
  let index = 0;
  for (const parameter of directParameters(callable)) {
    ...
    for (const name2 of names) {
      result.push({ name: name2, type: sourceText(type ?? parameter, source), index });
      index += 1;
    }
  }
  return result;
}
function receiverName(method, source) {
  ...
  return declaration === void 0 ? void 0 : declaredNames(declaration, source)[0];
}
function receiverType(method, source) {
  const receiver = method.childForFieldName("receiver");
  const declaration = receiver === null ? void 0 : descendants(receiver, "parameter_declaration")[0];
  ...
  return /^[A-Za-z_]\w*$/.test(normalized) ? normalized : void 0;
}
...
function declaredNames(declaration, source) {
  const type = declaration.childForFieldName("type");
  return declaration.namedChildren.filter((node) => (node.type === "identifier" || node.type === "field_identifier") && (type === null || node.endIndex <= type.startIndex)).map((node) => sourceText(node, source));
}
function packageName(root, source) {
  const clause = descendants(root, "package_clause")[0];
  const name2 = clause?.namedChildren.find((node) => node.type === "package_identifier" || node.type === "identifier");
  return name2 === void 0 ? "" : sourceText(name2, source);
}
function importsByAlias(root, source) {
...
function aliasForPath(imports, path) {
  for (const [alias, candidate] of imports) if (candidate === path) return alias;
  return void 0;
}
function importResolvesTo(importPath, target, modulePath) {
  if (modulePath === void 0) return false;
...
function insideDeferStatement(node) {
  let current = node.parent;
  while (current !== null) {
...
function changedEvidence(mode, file, node) {
  ...
  for (let line = start2; line <= end; line += 1) if (file.changedLines.has(line)) return true;
  return false;
}
function makeSignal(file, node, fact) {
...
async function analyzeDiscovery(discovery) {
  const signals = await analyzeAsyncListenerOwnership(discovery);
  const positives = [];
  const parseErrors = [];
  ...
  return {
    ...
    ...discovery.goVersion === void 0 ? {} : { goVersion: discovery.goVersion },
...
async function analyzeFile(file) {
  ...
  try {
    ...
    return {
      signals: signals.filter((item) => isChangedEvidence(file, item.line, item.endLine) || item.data.changeKind === "deletion"),
...
function analyzePrematureExternalStateMarker(file, root, previousRoot, signals) {
  ...
  const previous = previousRoot === void 0 || file.previous === void 0 ? /* @__PURE__ */ new Set() : new Set(collectPrematureStateMarkers(previousRoot, file.previous).map((fact) => fact.signature));
  for (const fact of current) {
    if (file.status === "modified" && previous.has(fact.signature)) continue;
    const anchor = fact.evidence.find((node) => isChangedEvidence(file, node.startPosition.row + 1, node.endPosition.row + 1));
...
function collectPrematureStateMarkers(root, source) {
  const facts = [];
  for (const fn of [
    ...descendants(root, "function_declaration"),
    ...
    const lists = functionDescendants(fn, body2, "statement_list");
...
function absentMapLookup(statement, source) {
  ...
  const condition = statement.childForFieldName("condition");
  if (initializer?.type !== "short_var_declaration" || condition === null) return void 0;
  const left = initializer.childForFieldName("left")?.namedChildren ?? [];
  const right = initializer.childForFieldName("right")?.namedChildren ?? [];
  if (left.length !== 2 || right.length !== 1 || right[0]?.type !== "index_expression") return void 0;
  const okName = sourceText(left[1], source);
  if (okName === "_" || !conditionMeansAbsent(condition, source, okName)) return void 0;
  const operand = right[0].childForFieldName("operand");
  const key = right[0].childForFieldName("index");
  if (operand === null || key === null) return void 0;
  return {
...
function conditionMeansAbsent(condition, source, okName) {
  const text = semanticText(sourceText(condition, source));
  const escaped = escapeRegExp(okName);
  return new RegExp(`^!${escaped}$`).test(text) || new RegExp(`^(?:${escaped}==false|false==${escaped})$`).test(text);
}
function fallthroughExternalFailure(fn, consequence, source, state) {
  for (const guard of directScopeDescendants(consequence, "if_statement")) {
    ...
    if (selected === void 0 || !externalMutationName(selected.field)) continue;
    const stateRoot = state.split(".")[0];
    const operationRoot = selected.operand.split(".")[0];
    if (stateRoot !== void 0 && operationRoot === stateRoot) continue;
    if (blockTerminates(failureBlock, fn, source, true)) continue;
    const failureStatements = failureBlock.namedChildren.find((node) => node.type === "statement_list")?.namedChildren ?? [];
    return { guard, call, condition, failureStatements };
  }
  return void 0;
}
function externalMutationName(name2) {
  return /(?:^|_)(?:Add|Apply|Commit|Create|Do|Install|Provision|Publish|Put|Replicate|Send|Start|Sync|Upload|Write)$/i.test(name2) || /(?:Add|Apply|Commit|Create|Install|Provision|Publish|Put|Replicate|Send|Start|Sync|Upload|Write)$/i.test(name2);
}
function sameMapEntryWrite(statement, source, state, key) {
  if (statement.type !== "assignment_statement") return void 0;
  const left = statement.childForFieldName("left")?.namedChildren ?? [];
  if (left.length !== 1 || left[0]?.type !== "index_expression") return void 0;
  const operand = left[0].childForFieldName("operand");
  const index = left[0].childForFieldName("index");
  if (operand === null || index === null) return void 0;
  return semanticText(sourceText(operand, source)) === semanticText(state) && semanticText(sourceText(index, source)) === semanticText(key) ? statement : void 0;
}
...
function analyzeGoroutineIDStateKeys(file, root, previousRoot, aliases, signals) {
  if (file.path.endsWith("_test.go")) return;
  const runtimeAlias = aliases.get("runtime");
  const strconvAlias = aliases.get("strconv");
  const stringsAlias = aliases.get("strings");
  if (runtimeAlias === void 0 || strconvAlias === void 0 || stringsAlias === void 0) return;
  ...
  for (const call of descendants(root, "call_expression")) {
    ...
    if (selected === void 0 || !["Store", "Load", "Delete"].includes(selected.field)) continue;
    if (!isMutableStateExpression(selected.operandNode, mutableState)) continue;
    const args2 = call.childForFieldName("arguments")?.namedChildren ?? [];
    const key = args2[0];
    if (key === void 0) continue;
    const helperCall2 = directHelperCall(key, file.current, idHelpers);
    if (helperCall2 === void 0) continue;
    const resolvedEvidence = goroutineIDChangedEvidence(
      file,
      key,
      selected.operandNode,
      mutableState,
      helperCall2,
      previousRoot
    );
    if (resolvedEvidence === void 0) continue;
    const dedupe = `${selected.operand}:${helperCall2.helper}`;
    if (seen.has(dedupe)) continue;
    seen.add(dedupe);
    signals.push(signal(
      file,
      resolvedEvidence.node,
  ...
  for (const index of descendants(root, "index_expression")) {
    ...
    const helperCall2 = directHelperCall(key, file.current, idHelpers);
    if (helperCall2 === void 0) continue;
    const resolvedEvidence = goroutineIDChangedEvidence(file, key, operand, mutableState, helperCall2, previousRoot);
    if (resolvedEvidence === void 0) continue;
    const dedupe = `${state}:${helperCall2.helper}`;
    if (seen.has(dedupe)) continue;
    seen.add(dedupe);
    signals.push(signal(
      file,
  ...
  for (const call of descendants(root, "call_expression")) {
    ...
    const state = args2[0];
    const key = args2[1];
    if (state === void 0 || key === void 0) continue;
    const stateText = sourceText(state, file.current);
    if (!isMutableStateExpression(state, mutableState)) continue;
    const helperCall2 = directHelperCall(key, file.current, idHelpers);
    if (helperCall2 === void 0) continue;
    const resolvedEvidence = goroutineIDChangedEvidence(file, key, state, mutableState, helperCall2, previousRoot);
    if (resolvedEvidence === void 0) continue;
    const dedupe = `${stateText}:${helperCall2.helper}`;
    if (seen.has(dedupe)) continue;
    seen.add(dedupe);
    signals.push(signal(
      file,
...
function goroutineIDHelpers(root, source, runtimeAlias, strconvAlias, stringsAlias) {
  const helpers = /* @__PURE__ */ new Map();
  for (const fn of descendants(root, "function_declaration")) {
    const name2 = fn.childForFieldName("name");
    const body2 = fn.childForFieldName("body");
    if (name2 === null || body2 === null) continue;
    const stackCalls = functionDescendants(fn, body2, "call_expression").filter((call) => {
      const functionNode = call.childForFieldName("function");
      ...
      const args2 = call.childForFieldName("arguments")?.namedChildren ?? [];
      return args2.length >= 2 && sourceText(args2[1], source) === "false";
    });
    ...
    const stackBuffer = stackArgs[0] === void 0 ? void 0 : sliceOperandIdentifier(stackArgs[0], source);
    const stackLength = assignedResult(stackCall, source);
    if (stackBuffer === void 0 || stackLength === void 0 || stackLength === "_" || stackArgs[0] === void 0 || !isFullSlice(stackArgs[0])) continue;
    const prefixCalls = functionDescendants(fn, body2, "call_expression").filter((call) => {
      const functionNode = call.childForFieldName("function");
      ...
      const args2 = call.childForFieldName("arguments")?.namedChildren ?? [];
      return args2.length === 2 && isStackString(args2[0], fn, source, stackBuffer, stackLength) && isGoroutinePrefixLiteral(args2[1], source);
    });
    if (prefixCalls.length !== 1) continue;
    const prefixCall = prefixCalls[0];
    if (identifierReassignedBetween(fn, body2, stackCall, prefixCall, source, stackBuffer) || identifierReassignedBetween(fn, body2, stackCall, prefixCall, source, stackLength)) continue;
    const prefixValue = assignedResult(prefixCall, source);
    if (prefixValue === void 0) continue;
    const integerParses = functionDescendants(fn, body2, "call_expression").flatMap((call) => {
      const functionNode = call.childForFieldName("function");
      ...
      if (firstArgument === void 0 || !goroutineIDParseInput(
        firstArgument,
        fn,
        source,
        prefixValue,
    ...
    if (parsedID === void 0) continue;
    const identityReturns = functionDescendants(fn, body2, "return_statement").filter((statement) => {
    ...
    if (parsedIDUnconditionallyOverwritten(fn, body2, integerParse.call, source, parsedID)) continue;
    if (hasTerminatingZeroGuard(fn, body2, integerParse.call, parsedID, source)) continue;
    helpers.set(sourceText(name2, source), {
...
function assignedResult(call, source) {
  ...
  if (assignment === null || assignment === void 0 || assignment.type !== "short_var_declaration" && assignment.type !== "assignment_statement") return void 0;
  const right = assignment.childForFieldName("right")?.namedChildren ?? [];
  const left = assignment.childForFieldName("left")?.namedChildren ?? [];
  if (right.length !== 1 || right[0]?.id !== call.id || left.length !== 1 || left[0]?.type !== "identifier") return void 0;
  return sourceText(left[0], source);
}
function sliceOperandIdentifier(node, source) {
  if (node.type !== "slice_expression") return void 0;
  const operand = node.childForFieldName("operand");
  return operand?.type === "identifier" ? sourceText(operand, source) : void 0;
}
function isFullSlice(node) {
  const start2 = node.childForFieldName("start");
  return node.type === "slice_expression" && isZeroOrOmitted(start2) && node.childForFieldName("end") === null && node.childForFieldName("capacity") === null;
}
function isStackString(node, fnBoundary, source, buffer, length) {
  if (node.type !== "call_expression") return false;
  const fn = node.childForFieldName("function");
  const args2 = node.childForFieldName("arguments")?.namedChildren ?? [];
  if (fn?.type !== "identifier" || sourceText(fn, source) !== "string" || args2.length !== 1 || isLocallyShadowedAt(fnBoundary, node, source, "string")) return false;
  const slice = args2[0];
  const start2 = slice.childForFieldName("start");
  const end = slice.childForFieldName("end");
  return sliceOperandIdentifier(slice, source) === buffer && isZeroOrOmitted(start2) && end?.type === "identifier" && sourceText(end, source) === length && slice.childForFieldName("capacity") === null;
}
function isZeroOrOmitted(node) {
  return node === null || node.type === "int_literal" && parseGoInteger(node.text) === 0;
}
function isGoroutinePrefixLiteral(node, source) {
...
function functionDescendants(fn, body2, type) {
  return descendants(body2, type).filter((node) => {
    let current = node.parent;
...
function findEnclosingCallable(node) {
  let current = node;
  while (current !== null) {
...
function callableParameterDeclarations(fn) {
...
function hasTerminatingZeroGuard(fn, body2, parser, parsedID, source) {
  const parserStatements = nearestAncestor2(parser, "statement_list");
  const returns = functionDescendants(fn, body2, "return_statement").filter((statement) => containsIdentifier(statement, source, parsedID));
  const assignments = identifierAssignments(fn, body2, source, parsedID);
  return functionDescendants(fn, body2, "if_statement").some((statement) => {
    ...
    const condition = statement.childForFieldName("condition");
    return condition !== null && isParsedIDZeroCondition(condition, parsedID, source) && guardTerminates(statement, fn, source, false) && returns.length > 0 && returns.every((returned) => returned.startIndex > statement.endIndex && returned.startIndex >= parserStatements.startIndex && returned.endIndex <= parserStatements.endIndex) && assignments.every((assignment) => assignment.startIndex <= statement.endIndex || returns.every((returned) => assignment.startIndex >= returned.startIndex) || assignmentPreservesNonzeroID(assignment, parsedID, source));
  });
}
function hasTerminatingKeyGuard(fn, body2, assignment, use, key, source) {
  return terminatingKeyGuards(fn, body2, assignment, use, key, source).length > 0;
}
function terminatingKeyGuards(fn, body2, assignment, use, key, source) {
  return functionDescendants(fn, body2, "if_statement").filter((statement) => {
    ...
    if (guardStatements?.type !== "statement_list" || use.startIndex < guardStatements.startIndex || use.endIndex > guardStatements.endIndex) return false;
    const condition = statement.childForFieldName("condition");
    const laterAssignments = identifierAssignments(fn, body2, source, key).filter((candidate) => candidate.startIndex > statement.endIndex && candidate.endIndex < use.startIndex && assignmentMutatesKeyBinding(fn, body2, assignment, candidate, use, key, source));
    return condition !== null && isParsedIDZeroCondition(condition, key, source) && guardTerminates(statement, fn, source, true) && laterAssignments.every((candidate) => assignmentPreservesNonzeroID(candidate, key, source));
  });
}
function assignmentMutatesKeyBinding(fn, body2, origin, assignment, use, name2, source) {
  if (assignment.type === "short_var_declaration") return declarationVisibleAt(assignment, use);
  const shadows = [
    ...functionDescendants(fn, body2, "short_var_declaration"),
    ...functionDescendants(fn, body2, "var_spec"),
    ...functionDescendants(fn, body2, "range_clause"),
    ...functionDescendants(fn, body2, "receive_statement"),
    ...functionDescendants(fn, body2, "type_switch_statement")
  ];
  return !shadows.some((declaration) => declaration.startIndex > origin.endIndex && declaration.startIndex < assignment.startIndex && controlBindingNames(declaration, source).includes(name2) && declarationVisibleAt(declaration, assignment) && !declarationVisibleAt(declaration, use));
}
function guardTerminates(statement, boundary, source, allowReturn) {
  const consequence = statement.childForFieldName("consequence");
...
function blockTerminationOutcomes2(block, boundary, source, allowReturn) {
  ...
  if (statementList === void 0) {
    ...
    return nested === void 0 ? pathContinues : statementTerminationOutcomes(nested, boundary, source, allowReturn);
  }
  let outcomes = pathContinues;
  for (const statement of statementList.namedChildren) {
    if ((outcomes & pathContinues) === 0) break;
    outcomes = outcomes & ~pathContinues | statementTerminationOutcomes(statement, boundary, source, allowReturn);
  }
  return outcomes;
}
function statementTerminationOutcomes(statement, boundary, source, allowReturn) {
  ...
  if (statement.type === "break_statement" || statement.type === "goto_statement" || statement.type === "fallthrough_statement") return pathTerminatesUnsafely;
  if (statement.type === "if_statement") {
    const consequence = statement.childForFieldName("consequence");
    const alternative = statement.childForFieldName("alternative");
  ...
  return callee?.type === "identifier" && sourceText(callee, source) === "panic" && isBuiltinPanic(call, boundary, source) ? pathTerminatesSafely : pathContinues;
}
function switchTerminationOutcomes(statement, boundary, source, allowReturn) {
  const cases = statement.namedChildren.filter((node) => node.type === "expression_case" || node.type === "type_case" || node.type === "default_case");
...
function isBuiltinPanic(call, boundary, source) {
  if (isLocallyShadowedAt(boundary, call, source, "panic")) return false;
  let root = boundary;
  while (root.parent !== null) root = root.parent;
  return !descendants(root, "function_declaration").some((fn) => {
    const name2 = fn.childForFieldName("name");
    return name2 !== null && sourceText(name2, source) === "panic";
  }) && !descendants(root, "var_spec").some((spec) => findEnclosingCallable(spec) === null && directDeclaredNames(spec, source).includes("panic"));
}
function isParsedIDZeroCondition(condition, parsedID, source) {
  ...
  return isExactIdentifierValue(left, parsedID, source) && integerNodeValue(right, source) === 0 && (operator === "==" || operator === "<=") || integerNodeValue(left, source) === 0 && isExactIdentifierValue(right, parsedID, source) && (operator === "==" || operator === ">=") || isExactIdentifierValue(left, parsedID, source) && integerNodeValue(right, source) === 1 && operator === "<";
}
...
function integerNodeValue(node, source) {
  const current = unwrapParentheses(node);
  if (current.type === "int_literal") return parseGoInteger(sourceText(current, source));
  if (current.type !== "unary_expression" || current.namedChildren.length !== 1) return void 0;
...
function relatedReassignments(fn, body2, origin, parse, source, name2, stringsAlias) {
  const originAssignment = origin.parent?.parent;
  if (originAssignment === null || originAssignment === void 0) return void 0;
  const related = [];
  let state = "trimmed";
  const assignments = [
    ...functionDescendants(fn, body2, "short_var_declaration"),
    ...functionDescendants(fn, body2, "assignment_statement")
  ].filter((assignment) => assignment.startIndex > origin.endIndex && assignment.endIndex < parse.startIndex).sort((left, right) => left.startIndex - right.startIndex);
  for (const assignment of assignments) {
    const left = assignment.childForFieldName("left")?.namedChildren ?? [];
    const indexes = left.flatMap((node, index) => node.type === "identifier" && sourceText(node, source) === name2 ? [index] : []);
...
function goroutineIDParseInput(node, fn, source, name2, stringsAlias, state) {
  const current = unwrapParentheses(node);
  if (isExactIdentifierValue(current, name2, source)) return state === "token";
  return isGoroutinePrefixTokenSelection(current, fn, source, name2, stringsAlias);
}
function isGoroutinePrefixTokenSelection(current, fn, source, name2, stringsAlias) {
  ...
  const args2 = operand.childForFieldName("arguments")?.namedChildren ?? [];
  if (calledText === `${stringsAlias}.Fields`) {
    return args2.length === 1 && isExactIdentifierValue(args2[0], name2, source);
  }
  return calledText === `${stringsAlias}.Split` && args2.length === 2 && isExactIdentifierValue(args2[0], name2, source) && isSpaceLiteral(args2[1], source);
}
function goroutinePrefixAssignmentState(value, assignment, origin, fn, body2, source, name2, stringsAlias, state) {
  const unwrapped = unwrapParentheses(value);
  if (isExactIdentifierValue(unwrapped, name2, source)) return state;
  if (isGoroutinePrefixTokenSelection(unwrapped, fn, source, name2, stringsAlias)) return "token";
  if (state !== "trimmed") return void 0;
  const current = unwrapParentheses(value);
  if (current.type !== "slice_expression") return void 0;
  const operand = current.childForFieldName("operand");
  const start2 = current.childForFieldName("start");
  const end = current.childForFieldName("end");
  if (operand === null || !isExactIdentifierValue(operand, name2, source) || !isZeroOrOmitted(start2) || end?.type !== "identifier" || current.childForFieldName("capacity") !== null) return void 0;
  const boundary = sourceText(end, source);
  const matched = functionDescendants(fn, body2, "call_expression").some((call) => {
    if (call.startIndex <= origin.endIndex || call.endIndex >= assignment.startIndex) return false;
    const called = call.childForFieldName("function");
    if (called === null || sourceText(called, source) !== `${stringsAlias}.IndexByte` || isLocallyShadowedAt(fn, call, source, stringsAlias)) return false;
    const args2 = call.childForFieldName("arguments")?.namedChildren ?? [];
    const boundaryOrigin = call.parent?.parent;
    if (boundaryOrigin === null || boundaryOrigin === void 0) return false;
    return args2.length === 2 && isExactIdentifierValue(args2[0], name2, source) && isSpaceLiteral(args2[1], source) && assignedResult(call, source) === boundary && declarationVisibleAt(boundaryOrigin, assignment) && !identifierAssignments(fn, body2, source, boundary).some((candidate) => candidate.id !== boundaryOrigin.id && candidate.startIndex > call.endIndex && candidate.endIndex < assignment.startIndex && assignmentMutatesKeyBinding(
      fn,
      body2,
      boundaryOrigin,
      candidate,
  ...
  return matched ? "token" : void 0;
}
function isSpaceLiteral(node, source) {
  if (node.type !== "interpreted_string_literal" && node.type !== "raw_string_literal" && node.type !== "rune_literal") return false;
  return ['" "', "` `", "' '"].includes(sourceText(node, source));
}
function identifierReassignedBetween(fn, body2, from, to, source, name2) {
  return identifierAssignments(fn, body2, source, name2).some((assignment) => assignment.startIndex > from.endIndex && assignment.endIndex < to.startIndex);
}
function parsedIDUnconditionallyOverwritten(fn, body2, parser, source, name2) {
  const statements = nearestAncestor2(parser, "statement_list");
  if (statements === null) return false;
  const returns = functionDescendants(fn, body2, "return_statement").filter((statement) => containsIdentifier(statement, source, name2));
  return identifierAssignments(fn, body2, source, name2).some((assignment) => {
    if (assignment.startIndex <= parser.endIndex || assignment.parent?.id !== statements.id) return false;
    const left = assignment.childForFieldName("left")?.namedChildren ?? [];
    const right = assignment.childForFieldName("right")?.namedChildren ?? [];
    const offset = left.findIndex((node) => node.type === "identifier" && sourceText(node, source) === name2);
    const value = right.length === left.length ? right[offset] : right[0];
    return value !== void 0 && !assignmentUsesPreviousBinding(assignment, name2, source) && !containsIdentifier(value, source, name2) && returns.length > 0 && returns.every((returned) => returned.startIndex > assignment.endIndex && returned.startIndex >= statements.startIndex && returned.endIndex <= statements.endIndex);
  });
}
function identifierAssignments(fn, body2, source, name2) {
  return [
    ...functionDescendants(fn, body2, "short_var_declaration"),
    ...functionDescendants(fn, body2, "assignment_statement")
  ].filter((assignment) => {
    const left = assignment.childForFieldName("left")?.namedChildren ?? [];
    return left.some((node) => node.type === "identifier" && sourceText(node, source) === name2);
  });
}
function assignmentPreservesNonzeroID(assignment, name2, source) {
  const left = assignment.childForFieldName("left")?.namedChildren ?? [];
  const right = assignment.childForFieldName("right")?.namedChildren ?? [];
  const offset = left.findIndex((node) => node.type === "identifier" && sourceText(node, source) === name2);
  const value = right.length === left.length ? right[offset] : right[0];
  ...
  if (operator !== void 0 && operator !== "=" && operator !== ":=") {
    const constant2 = integerNodeValue(value, source);
    if ((operator === "|=" || operator === "&=") && isExactIdentifierValue(value, name2, source)) return true;
    return (operator === "+=" || operator === "-=" || operator === "|=" || operator === "^=" || operator === "<<=" || operator === ">>=" || operator === "&^=") && constant2 === 0 || (operator === "*=" || operator === "/=") && constant2 === 1;
  }
  if (expressionPreservesIDIdentity(value, name2, source)) return true;
  const constant = integerNodeValue(value, source);
  return constant !== void 0 && constant !== 0;
}
function assignmentUsesPreviousBinding(assignment, name2, source) {
  if (assignment.type !== "assignment_statement") return false;
  const left = assignment.childForFieldName("left")?.namedChildren ?? [];
  return left.length === 1 && left[0]?.type === "identifier" && sourceText(left[0], source) === name2 && ![void 0, "=", ":="].includes(assignmentOperator(assignment, source));
}
function assignmentOperator(assignment, source) {
  const left = assignment.childForFieldName("left");
  const right = assignment.childForFieldName("right");
  if (left === null || right === null) return void 0;
  const operator = source.slice(left.endIndex, right.startIndex).trim();
  return /^(?::=|=|\+=|-=|\*=|\/=|%=|&=|\|=|\^=|<<=|>>=|&\^=)$/.test(operator) ? operator : void 0;
}
function expressionPreservesIDIdentity(value, name2, source) {
  if (isExactIdentifierValue(value, name2, source)) return true;
  if (isBuiltinInt64IdentityConversion(value, name2, source)) return true;
  ...
  if ((operator === "|" || operator === "&") && isExactIdentifierValue(first, name2, source) && isExactIdentifierValue(second, name2, source)) return true;
  if ((operator === "+" || operator === "|") && (isExactIdentifierValue(first, name2, source) && integerNodeValue(second, source) === 0 || integerNodeValue(first, source) === 0 && isExactIdentifierValue(second, name2, source))) return true;
  if (operator === "-" && isExactIdentifierValue(first, name2, source) && integerNodeValue(second, source) === 0) return true;
  if (operator === "/" && isExactIdentifierValue(first, name2, source) && integerNodeValue(second, source) === 1) return true;
  if (operator === "^" && isExactIdentifierValue(first, name2, source) && integerNodeValue(second, source) === 0) return true;
  return operator === "*" && (isExactIdentifierValue(first, name2, source) && integerNodeValue(second, source) === 1 || integerNodeValue(first, source) === 1 && isExactIdentifierValue(second, name2, source));
}
function isBuiltinInt64IdentityConversion(node, name2, source) {
  const current = unwrapParentheses(node);
  if (current.type !== "call_expression") return false;
  const fn = current.childForFieldName("function");
  const args2 = current.childForFieldName("arguments")?.namedChildren ?? [];
  if (fn?.type !== "identifier" || sourceText(fn, source) !== "int64" || args2.length !== 1 || !isExactIdentifierValue(args2[0], name2, source)) return false;
...
function isExactIdentifierValue(node, name2, source) {
  if (node.type === "identifier") return sourceText(node, source) === name2;
  if (node.type === "parenthesized_expression" && node.namedChildren.length === 1) {
    return isExactIdentifierValue(node.namedChildren[0], name2, source);
  }
...
function isLocallyShadowedAt(fn, reference, source, name2) {
  const body2 = fn.childForFieldName("body");
  const declarations = [
  ...
  for (const declaration of declarations) {
    if (declaration.startIndex >= reference.startIndex || !callableParameterDeclarations(fn).some((parameter) => parameter.id === declaration.id) && !declarationVisibleAt(declaration, reference)) continue;
    if (controlBindingNames(declaration, source).includes(name2)) return true;
  }
  return false;
}
function controlBindingNames(declaration, source) {
  if (declaration.type === "parameter_declaration" || declaration.type === "var_spec") {
    return directDeclaredNames(declaration, source);
  }
  if ((declaration.type === "range_clause" || declaration.type === "receive_statement") && !sourceText(declaration, source).includes(":=")) return [];
  ...
  return named.filter((node) => node.type === "identifier" || node.type === "field_identifier").map((node) => sourceText(node, source));
}
function escapeRegExp(value) {
  return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
function changedHelperAnchor(file, anchors, previousRoot) {
  return anchors.find((node) => isSemanticallyChangedEvidence(file, node, previousRoot));
}
function goroutineIDChangedEvidence(file, key, state, mutableState, helper, previousRoot) {
  ...
  if (helperAnchor !== void 0) return { node: helperAnchor, metadata: {} };
  const guardAnchor = changedTerminatingGuardEvidence(file, helper, previousRoot);
  if (guardAnchor !== void 0) return guardAnchor;
  const callerGuardAnchor = changedCallerGuardEvidence(file, helper, previousRoot);
  if (callerGuardAnchor !== void 0) return callerGuardAnchor;
  const stateAnchor = mutableStateEvidence(state, mutableState).find((node) => isSemanticallyChangedEvidence(file, node, previousRoot));
  if (stateAnchor !== void 0) return { node: stateAnchor, metadata: { changeKind: "state-declaration" } };
  return deletedTerminatingGuardEvidence(file, helper, previousRoot);
}
function changedCallerGuardEvidence(file, helper, previousRoot) {
  const caller = helper.callerKey;
  if (caller === void 0 || file.status !== "modified" || file.previous === void 0 || previousRoot === void 0) {
    return void 0;
  }
  ...
  if (previousScope === void 0 || previousBody === null || previousBody === void 0) return void 0;
  ...
  const proof = previousOrigins.flatMap((origin) => previousUses.filter((use) => use.startIndex > origin.endIndex).flatMap((use) => terminatingKeyGuards(
    previousScope,
    previousBody,
    origin,
    use,
  ...
  if (currentBody !== null) {
    const changedGuard = functionDescendants(caller.scope, currentBody, "if_statement").find((statement) => {
      ...
      const condition = statement.childForFieldName("condition");
      return condition !== null && isParsedIDZeroCondition(condition, caller.key, file.current) && isSemanticallyChangedEvidence(file, statement, previousRoot);
    });
    if (changedGuard !== void 0) {
...
function callableIdentity(callable, source) {
  const name2 = callable.childForFieldName("name");
  if (name2 === null) return void 0;
  if (callable.type === "function_declaration") return `function:${sourceText(name2, source)}`;
  if (callable.type !== "method_declaration") return void 0;
  const receiver = callable.childForFieldName("receiver");
  const declaration = receiver === null ? void 0 : descendants(receiver, "parameter_declaration")[0];
  const type = declaration?.childForFieldName("type");
  return type === null || type === void 0 ? void 0 : `method:${sourceText(type, source).replace(/\s+/g, "")}:${sourceText(name2, source)}`;
}
function assignmentAssignsHelper(assignment, key, helper, source) {
  const left = assignment.childForFieldName("left")?.namedChildren ?? [];
  const right = assignment.childForFieldName("right")?.namedChildren ?? [];
...
function mutableStateKeyUses(callable, body2, key, index) {
  const uses = [];
  for (const call of functionDescendants(callable, body2, "call_expression")) {
    const selected = selectedCall(call, index.source);
    if (selected !== void 0 && ["Store", "Load", "Delete"].includes(selected.field) && isMutableStateExpression(selected.operandNode, index)) {
      const candidate = call.childForFieldName("arguments")?.namedChildren[0];
      if (candidate !== void 0 && isExactIdentifierValue(candidate, key, index.source)) uses.push(candidate);
      continue;
    }
    const functionNode = call.childForFieldName("function");
    if (functionNode?.type !== "identifier" || sourceText(functionNode, index.source) !== "delete") continue;
    const args2 = call.childForFieldName("arguments")?.namedChildren ?? [];
    if (args2[0] !== void 0 && args2[1] !== void 0 && isMutableStateExpression(args2[0], index) && isExactIdentifierValue(args2[1], key, index.source)) uses.push(args2[1]);
  }
  for (const expression of functionDescendants(callable, body2, "index_expression")) {
    const operand = expression.childForFieldName("operand");
    const candidate = expression.childForFieldName("index");
    if (operand !== null && candidate !== null && isMutableStateExpression(operand, index) && isExactIdentifierValue(candidate, key, index.source)) uses.push(candidate);
  }
  return uses;
}
function changedTerminatingGuardEvidence(file, helper, previousRoot) {
  if (file.status !== "modified" || file.previous === void 0 || previousRoot === void 0) return void 0;
  const currentBody = helper.scope.childForFieldName("body");
  if (currentBody === null) return void 0;
  for (const statement of functionDescendants(helper.scope, currentBody, "if_statement")) {
    ...
    const previousHelper = previous === void 0 ? null : findEnclosingCallable(previous);
    if (previous !== void 0 && previousCondition !== null && previousCondition !== void 0 && previousHelper !== null && isParsedIDZeroCondition(previousCondition, helper.parsedID, file.previous) && guardTerminates(previous, previousHelper, file.previous, false)) {
      return { node: statement, metadata: { changeKind: "guard-change" } };
    }
  }
  return void 0;
}
function deletedTerminatingGuardEvidence(file, helper, previousRoot) {
  if (file.status !== "modified" || file.previous === void 0 || previousRoot === void 0) return void 0;
  const currentName = helper.scope.childForFieldName("name");
  const currentBody = helper.scope.childForFieldName("body");
  if (currentName === null || currentBody === null) return void 0;
  ...
  const previousHelper = descendants(previousRoot, "function_declaration").find((fn) => {
    const name2 = fn.childForFieldName("name");
    return name2 !== null && sourceText(name2, file.previous) === helperName;
  });
  const previousBody = previousHelper?.childForFieldName("body");
  if (previousHelper === void 0 || previousBody === null || previousBody === void 0) return void 0;
  const currentGuardSignatures = new Set(
    functionDescendants(helper.scope, currentBody, "if_statement").map((statement) => semanticNodeSignature(statement, file.current))
  );
  const deletedGuard = functionDescendants(previousHelper, previousBody, "if_statement").find((statement) => {
    const condition = statement.childForFieldName("condition");
    return condition !== null && isParsedIDZeroCondition(condition, helper.parsedID, file.previous) && guardTerminates(statement, previousHelper, file.previous, false) && !currentGuardSignatures.has(semanticNodeSignature(statement, file.previous));
  });
  if (deletedGuard === void 0) return void 0;
  return {
    node: currentName,
    metadata: {
      changeKind: "deletion",
...
function discardedParseResult(call, source) {
  let current = call.parent;
...
function mutableStateIndex(root, source, syncAlias) {
  const mapAliases = /* @__PURE__ */ new Set();
  const mapAliasDeclarations = /* @__PURE__ */ new Map();
  ...
  while (discovered) {
    ...
    for (const spec of typeSpecs) {
      ...
      const alias = sourceText(name2, source);
  ...
  const fieldDeclarations = /* @__PURE__ */ new Map();
  for (const spec of typeSpecs) {
    const name2 = spec.childForFieldName("name");
    const type = spec.childForFieldName("type");
    if (name2 === null || type?.type !== "struct_type") continue;
    ...
    for (const field of descendants(type, "field_declaration")) {
      ...
      for (const fieldName of directDeclaredNames(field, source)) {
        const names = fields.get(typeName) ?? /* @__PURE__ */ new Set();
        names.add(fieldName);
        fields.set(typeName, names);
        const declarations = fieldDeclarations.get(typeName) ?? /* @__PURE__ */ new Map();
...
function isMutableStateExpression(node, index) {
  ...
  const receiverType2 = binding?.namedType;
  return binding?.receiverOwned === true && receiverType2 !== void 0 && index.fields.get(receiverType2)?.has(sourceText(field, index.source)) === true;
}
function mutableStateEvidence(node, index) {
  if (node.type === "identifier") {
  ...
  const receiverType2 = receiverBinding?.namedType;
  if (receiverBinding?.receiverOwned !== true || receiverType2 === void 0) return [];
  const declaration = index.fieldDeclarations.get(receiverType2)?.get(sourceText(field, index.source));
  return declaration === void 0 ? [] : [...receiverBinding?.evidence ?? [], declaration];
}
function resolveIdentifierBinding(name2, use, index, excludedDeclarationId) {
  const callable = findEnclosingCallable(use);
  if (callable !== null) {
    const body2 = callable.childForFieldName("body");
    if (body2 !== null) {
      const locals = [
        ...functionDescendants(callable, body2, "var_spec"),
        ...functionDescendants(callable, body2, "short_var_declaration"),
        ...functionDescendants(callable, body2, "assignment_statement"),
        ...functionDescendants(callable, body2, "range_clause"),
        ...functionDescendants(callable, body2, "receive_statement"),
        ...functionDescendants(callable, body2, "type_switch_statement")
      ].filter((declaration) => declaration.id !== excludedDeclarationId && declaration.startIndex < use.startIndex && declarationVisibleAt(declaration, use)).flatMap((declaration) => {
        const binding = bindingFromDeclaration(declaration, name2, index);
        return binding === void 0 ? [] : [{ binding, start: declaration.startIndex }];
      }).sort((left, right) => right.start - left.start);
      if (locals[0] !== void 0) return locals[0].binding;
    }
    for (const parameter of callableParameterDeclarations(callable)) {
      if (!directDeclaredNames(parameter, index.source).includes(name2)) continue;
      const type = parameter.childForFieldName("type");
      if (type !== null) return bindingFromType(type, index, parameter);
    }
  }
  const globals = descendants(index.root, "var_spec").filter((spec) => spec.id !== excludedDeclarationId && findEnclosingCallable(spec) === null).flatMap((spec) => {
    const binding = bindingFromDeclaration(spec, name2, index);
    return binding === void 0 ? [] : [{ binding, start: spec.startIndex }];
  }).sort((left, right) => right.start - left.start);
  return globals[0]?.binding;
}
function bindingFromDeclaration(declaration, name2, index) {
  if (declaration.type === "var_spec") {
    const names = directDeclaredNames(declaration, index.source);
    const offset = names.indexOf(name2);
    ...
    return value === void 0 ? { mutable: false, evidence: [declaration] } : bindingFromValue(value, index, declaration);
  }
  if (declaration.type === "short_var_declaration" || declaration.type === "assignment_statement") {
    const left = declaration.childForFieldName("left")?.namedChildren ?? [];
    const right = declaration.childForFieldName("right")?.namedChildren ?? [];
    const offset = left.findIndex((node) => node.type === "identifier" && sourceText(node, index.source) === name2);
    if (offset < 0) return void 0;
    const value = right.length === left.length ? right[offset] : right[0];
    return value === void 0 ? { mutable: false, evidence: [declaration] } : bindingFromValue(value, index, declaration);
  }
  if (declaration.type === "range_clause" || declaration.type === "receive_statement" || declaration.type === "type_switch_statement") {
    return controlBindingNames(declaration, index.source).includes(name2) ? { mutable: false, evidence: [declaration] } : void 0;
  }
  return void 0;
}
function bindingFromType(type, index, declaration) {
  const text = sourceText(type, index.source);
  const resolved = namedType(text);
  const alias = resolved === void 0 ? void 0 : index.mapAliasDeclarations.get(resolved);
  return {
    mutable: isMutableStateType(text, index.syncAlias, index.mapAliases),
    ...resolved === void 0 ? {} : { namedType: resolved },
    ...isMethodReceiverDeclaration(declaration) ? { receiverOwned: true } : {},
    evidence: alias === void 0 ? [declaration] : [declaration, alias]
  };
}
function isMethodReceiverDeclaration(declaration) {
  let current = declaration.parent;
...
function bindingFromValue(value, index, declaration) {
  const aliased = aliasedMutableStateBinding(value, index, declaration);
  if (aliased?.mutable === true || aliased?.namedType !== void 0) {
  ...
  const alias = type === void 0 ? void 0 : index.mapAliasDeclarations.get(type);
  return {
    mutable: isMutableStateInitializer(text, index.syncAlias, index.mapAliases),
    ...type === void 0 ? {} : { namedType: type },
    evidence: alias === void 0 ? [declaration] : [declaration, alias]
  };
}
function aliasedMutableStateBinding(value, index, declaration) {
  if (value.type === "identifier") {
    return resolveIdentifierBinding(sourceText(value, index.source), value, index, declaration.id);
  }
  if (value.type !== "selector_expression") return void 0;
  const receiver = value.childForFieldName("operand");
  const field = value.childForFieldName("field");
  if (receiver?.type !== "identifier" || field === null) return void 0;
  const receiverType2 = resolveIdentifierBinding(
    sourceText(receiver, index.source),
    value,
    index,
    declaration.id
  )?.namedType;
  if (receiverType2 === void 0) return void 0;
  const fieldName = sourceText(field, index.source);
  const fieldDeclaration = index.fieldDeclarations.get(receiverType2)?.get(fieldName);
  return fieldDeclaration === void 0 ? void 0 : { mutable: true, evidence: [fieldDeclaration] };
}
function directDeclaredNames(declaration, source) {
  const type = declaration.childForFieldName("type");
  return declaration.namedChildren.filter((node) => (node.type === "identifier" || node.type === "field_identifier") && (type === null || node.endIndex <= type.startIndex)).map((node) => sourceText(node, source));
}
function declarationVisibleAt(declaration, use) {
  let scope = declaration.parent;
  const lexicalScopes = /* @__PURE__ */ new Set([
    "block",
    "source_file",
    "if_statement",
    "for_statement",
...
function nearestAncestor2(node, type) {
  let current = node.parent;
  while (current !== null) {
    if (current.type === type) return current;
    current = current.parent;
...
function initializedNamedType(value) {
  return /^&?([A-Za-z_]\w*)\s*\{/.exec(value.trim())?.[1];
}
function isMutableStateType(type, syncAlias, mapAliases) {
  const normalized = type.replace(/\s+/g, "");
  return /^map\[/.test(normalized) || mapAliases.has(normalized.replace(/[()]/g, "")) || syncAlias !== void 0 && (normalized === `${syncAlias}.Map` || normalized === `*${syncAlias}.Map`);
}
function isMutableStateInitializer(value, syncAlias, mapAliases) {
  const initialized = initializedNamedType(value);
  const made = /^make\s*\(\s*([A-Za-z_]\w*)\s*(?:,|\))/.exec(value)?.[1];
  return /^make\s*\(\s*map\s*\[/.test(value) || /^map\s*\[.*\]\s*.*\{/.test(value) || made !== void 0 && mapAliases.has(made) || initialized !== void 0 && mapAliases.has(initialized) || syncAlias !== void 0 && new RegExp(`^&?${syncAlias.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\.Map\\s*\\{`).test(value);
}
function directHelperCall(key, source, helpers) {
  if (key.type === "call_expression") return helperCall(key, source, helpers);
  if (key.type !== "identifier") return void 0;
  const enclosing = findEnclosingCallable(key);
  const body2 = enclosing?.childForFieldName("body");
  if (enclosing === null || enclosing === void 0 || body2 === null || body2 === void 0) return void 0;
  const keyName = sourceText(key, source);
  const assignments = [
    ...functionDescendants(enclosing, body2, "short_var_declaration"),
    ...functionDescendants(enclosing, body2, "assignment_statement")
  ].filter((assignment) => assignment.startIndex < key.startIndex && declarationVisibleAt(assignment, key)).flatMap((assignment) => {
    const left = assignment.childForFieldName("left")?.namedChildren ?? [];
    const right = assignment.childForFieldName("right")?.namedChildren ?? [];
  ...
  const origins = assignments.flatMap((candidate) => {
    ...
    return resolved === void 0 ? [] : [{ ...candidate, resolved }];
  });
  ...
  if (transformations.some((candidate) => candidate.value === void 0 || !assignmentUsesPreviousBinding(candidate.assignment, keyName, source) && !containsIdentifier(candidate.value, source, keyName) && integerNodeValue(candidate.value, source) !== 0)) {
    return void 0;
  }
  if (hasTerminatingKeyGuard(enclosing, body2, origin.assignment, key, keyName, source)) return void 0;
  return {
    ...origin.resolved,
...
function helperCall(call, source, helpers) {
  const functionNode = call.childForFieldName("function");
  if (functionNode?.type !== "identifier") return void 0;
...
function analyzeContextDetachment(file, root, aliases, signals) {
  ...
  for (const fn of callables) {
    const body2 = fn.childForFieldName("body");
    if (body2 === null) continue;
    const parentContexts = parameterDeclarations(fn).filter((parameter) => {
      const type = parameter.childForFieldName("type");
      return type !== null && sourceText(type, file.current) === `${contextAlias}.Context`;
    }).map((parameter) => parameter.childForFieldName("name")).filter((name2) => name2 !== null).map((name2) => sourceText(name2, file.current)).filter((name2) => name2 !== "_");
    if (parentContexts.length === 0) continue;
    for (const call of directScopeDescendants(body2, "call_expression")) {
      const functionNode = call.childForFieldName("function");
...
function directScopeDescendants(node, type) {
  const result = [];
  const pending = [...node.namedChildren].reverse();
  while (pending.length > 0) {
    const current = pending.pop();
    if (current === void 0 || current.type === "func_literal") continue;
    if (current.type === type) result.push(current);
    for (let index = current.namedChildCount - 1; index >= 0; index -= 1) {
      const child = current.namedChild(index);
      if (child !== null) pending.push(child);
...
function isChangedEvidence(file, line, endLine = line) {
...
function isSemanticallyChangedEvidence(file, node, previousRoot) {
  if (!isChangedEvidence(file, node.startPosition.row + 1, node.endPosition.row + 1)) return false;
  if (file.status !== "modified" || file.previous === void 0 || previousRoot === void 0) return true;
  const previous = correspondingNodeIgnoringComments(node, previousRoot);
  return previous === void 0 || semanticNodeSignature(node, file.current) !== semanticNodeSignature(previous, file.previous);
}
function correspondingNodeIgnoringComments(node, previousRoot) {
  const path = [];
  let current = node;
  ...
  return previous.type === node.type ? previous : void 0;
}
function nonCommentChildren(node) {
  const children = [];
...
function semanticNodeSignature(node, source) {
...
function analyzeFunction(file, fn, aliases, atomicAccessors, signals, positives) {
  const body2 = fn.childForFieldName("body");
  const syncAlias = aliases.get("sync");
  analyzeCopyLocks(file, fn, syncAlias, signals);
  if (body2 === null) return;
  if (syncAlias !== void 0) analyzeWaitGroups(file, body2, syncAlias, signals, positives);
  analyzeCancellation(file, body2, aliases, signals, positives);
  analyzeContextErrorClassification(file, fn, body2, aliases, signals);
  analyzeAtomicCapacityAdmission(file, fn, body2, aliases, atomicAccessors, signals);
  analyzeChannels(file, body2, signals);
  analyzeLoopVarCapture(file, body2, signals);
  analyzeSelectDefaultBusy(file, body2, signals);
  analyzeTickers(file, fn, body2, aliases, signals);
  analyzeTimers(file, body2, aliases, signals);
}
function atomicAccessorMethods(root, source, aliases) {
  ...
  for (const method of descendants(root, "method_declaration")) {
    ...
    const body2 = method.childForFieldName("body");
    if (name2 === null || body2 === null) continue;
    const returns = descendants(body2, "return_statement");
    if (returns.length !== 1) continue;
    const loads = descendants(returns[0], "call_expression").map((call) => selectedCall(call, source)).filter((call) => call !== void 0 && call.field === "Load");
    if (loads.length !== 1) continue;
    const field = loads[0].operand.split(".").at(-1);
    if (field !== void 0) result.set(sourceText(name2, source), field);
  }
  return result;
}
function analyzeAtomicCapacityAdmission(file, fn, body2, aliases, atomicAccessors, signals) {
  ...
  const hasLock = descendants(body2, "call_expression").some((call) => {
    const selected = selectedCall(call, file.current);
    return selected !== void 0 && (selected.field === "Lock" || selected.field === "RLock");
  });
  if (hasLock) return;
  for (const guard of descendants(body2, "if_statement")) {
    const condition = guard.childForFieldName("condition") ?? guard.namedChildren[0];
    const consequence = guard.childForFieldName("consequence") ?? guard.namedChildren.find((node) => node.type === "block");
    if (condition === void 0 || consequence === void 0) continue;
    ...
    for (const call of descendants(condition, "call_expression")) {
      const selected2 = selectedCall(call, file.current);
      if (selected2 === void 0) continue;
      if (selected2.field === "Load") {
        checkedFields.add(selected2.operand);
        continue;
      }
      const accessorField = atomicAccessors.get(selected2.field);
      if (accessorField !== void 0) checkedFields.add(`${selected2.operand}.${accessorField}`);
    }
    if (checkedFields.size === 0) continue;
    const mutation = descendants(body2, "call_expression").find((call) => {
      if (call.startIndex <= guard.endIndex) return false;
      const selected2 = selectedCall(call, file.current);
      return selected2 !== void 0 && (selected2.field === "Add" || selected2.field === "Store" || selected2.field === "Swap") && checkedFields.has(selected2.operand);
    });
    if (mutation === void 0) continue;
...
function selectedCall(call, source) {
  ...
  const operand = fn.childForFieldName("operand");
  const field = fn.childForFieldName("field");
  if (operand === null || field === null) return void 0;
  return { operand: sourceText(operand, source), operandNode: operand, field: sourceText(field, source) };
}
function analyzeCopyLocks(file, fn, syncAlias, signals) {
  if (syncAlias === void 0) return;
  const waitGroupType = `${syncAlias}.WaitGroup`;
  const mutexTypes = /* @__PURE__ */ new Set([`${syncAlias}.Mutex`, `${syncAlias}.RWMutex`]);
  for (const parameter of parameterDeclarations(fn)) {
    ...
    if (typeText === waitGroupType) {
      ...
      signals.push(signal(
        ...
        `WaitGroup is passed by value${name2 === null ? "" : ` as parameter ${sourceText(name2, file.current)}`}; copies break Add/Done/Wait counter sharing.`,
      ...
      signals.push(signal(
        ...
        `${typeText} is passed by value${name2 === null ? "" : ` as parameter ${sourceText(name2, file.current)}`}; mutex copies unlock independently of the original.`,
  ...
  const body2 = fn.childForFieldName("body");
  if (body2 === null) return;
  const mutexNames = /* @__PURE__ */ new Set();
  for (const parameter of parameterDeclarations(fn)) {
    const typeNode = parameter.childForFieldName("type");
  ...
  for (const declaration of descendants(body2, "var_spec")) {
    const type = declaration.childForFieldName("type");
    const name2 = declaration.childForFieldName("name");
    if (type !== null && name2 !== null && mutexTypes.has(sourceText(type, file.current))) {
      mutexNames.add(sourceText(name2, file.current));
    }
  }
  for (const declaration of descendants(body2, "short_var_declaration")) {
...
function parameterDeclarations(fn) {
  const list = fn.childForFieldName("parameters");
  if (list === null) return [];
  return list.namedChildren.filter((node) => node.type === "parameter_declaration");
}
function analyzeWaitGroups(file, body2, syncAlias, signals, positives) {
  const waitGroups = /* @__PURE__ */ new Set();
  const waitGroupType = `${syncAlias}.WaitGroup`;
  for (const declaration of descendants(body2, "var_spec")) {
    const type = declaration.childForFieldName("type");
    const names = declaration.namedChildren.filter((node) => node.type === "identifier").map((node) => sourceText(node, file.current));
    if (type !== null && sourceText(type, file.current) === waitGroupType) {
      names.filter((name2) => name2 !== "_").forEach((name2) => waitGroups.add(name2));
      continue;
    }
  ...
  for (const name2 of waitGroups) {
    let unsafeAdd;
    for (const statement of descendants(body2, "go_statement")) {
      const add = selectorCalls(statement, file.current, name2, "Add")[0];
      if (add !== void 0) {
        unsafeAdd = add;
        signals.push(signal(
          file,
          ...
          `${name2}.Add runs inside the goroutine it is intended to register.`,
          { waitGroup: name2, operation: "Add", placement: "inside-goroutine" }
        ));
      }
    }
    if (unsafeAdd === void 0 && hasOrderedWaitGroupLifecycle(body2, file.current, name2)) {
      const add = selectorCalls(body2, file.current, name2, "Add")[0];
      if (add !== void 0) {
        positives.push({
          key: "go-concurrency.waitgroup-ordered",
          path: file.path,
          line: add.startPosition.row + 1,
          summary: `The ${name2} WaitGroup registers work before launch, defers Done in the worker, and waits after launch.`
        });
      }
    }
    analyzeWaitGroupCompletion(file, body2, name2, signals);
  }
}
function addInitializedWaitGroups(waitGroups, names, values, source, waitGroupType) {
  ...
  values.forEach((value, index) => {
    ...
    if (name2 !== void 0 && name2 !== "_" && initializer.test(sourceText(value, source))) {
      waitGroups.add(name2);
    }
  });
}
function analyzeWaitGroupCompletion(file, body2, name2, signals) {
  for (const goStatement of descendants(body2, "go_statement")) {
    const closure = goStatement.namedChildren.find((child) => child.type === "func_literal") ?? descendants(goStatement, "func_literal")[0];
    if (closure === void 0) continue;
    const closureBody = closure.childForFieldName("body");
    if (closureBody === null) continue;
    const doneCalls = directScopeDescendants(closureBody, "call_expression").filter((call) => isSelectorCall(call, file.current, name2, "Done"));
    ...
    for (const done of doneCalls) {
      ...
      if (earlierReturn === void 0) continue;
      signals.push(signal(
        file,
        earlierReturn,
        "go-concurrency.waitgroup.done-not-deferred",
        `${name2}.Done is reached only after an earlier return path in this goroutine; Wait can remain blocked when that path exits.`,
        {
          waitGroup: name2,
          operation: "Done",
...
function isSelectorCall(call, source, receiver, field) {
  const fn = call.childForFieldName("function");
  if (fn?.type !== "selector_expression") return false;
  const operand = fn.childForFieldName("operand");
  const selected = fn.childForFieldName("field");
  return operand !== null && selected !== null && sourceText(operand, source) === receiver && sourceText(selected, source) === field;
}
function isDeferredCall(call) {
  return call.parent?.type === "defer_statement";
}
function isInsideDeferredClosure(node, boundary) {
  let current = node.parent;
  while (current !== null && current.id !== boundary.id) {
...
function hasOrderedWaitGroupLifecycle(body2, source, name2) {
  const add = selectorCalls(body2, source, name2, "Add")[0];
  const wait = selectorCalls(body2, source, name2, "Wait")[0];
  const go = descendants(body2, "go_statement").find((statement) => selectorCalls(statement, source, name2, "Done").length > 0);
  return add !== void 0 && go !== void 0 && wait !== void 0 && add.startIndex < go.startIndex && go.startIndex < wait.startIndex;
}
function analyzeCancellation(file, body2, aliases, signals, positives) {
  const contextAlias = aliases.get("context");
  ...
  for (const helper of ["WithCancel", "WithCancelCause", "WithDeadline", "WithDeadlineCause", "WithTimeout", "WithTimeoutCause"]) {
    if (contextAlias !== void 0) helpers.set(`${contextAlias}.${helper}`, { kind: "cancel", helper });
  }
  if (errgroupAlias !== void 0) helpers.set(`${errgroupAlias}.WithContext`, { kind: "errgroup", helper: "WithContext" });
  for (const assignment of [
    ...
    if (lifecycleName === "_") {
      const message = helper.kind === "cancel" ? `${sourceText(functionNode, file.current)} discards its cancellation function.` : `${sourceText(functionNode, file.current)} discards the derived context used to propagate peer failure.`;
    ...
    } else if (helper.kind === "cancel" && callsIdentifier(body2, file.current, lifecycleName)) {
      positives.push({
    ...
    } else if (helper.kind === "cancel" && !identifierUsedOutsideBinding(body2, file.current, lifecycleName, left)) {
      signals.push(signal(
        file,
        assignment,
...
function analyzeContextErrorClassification(file, fn, body2, aliases, signals) {
  const contextAlias = aliases.get("context");
  if (contextAlias === void 0) return;
  const contextNames = new Set(
    parameterDeclarations(fn).filter((parameter) => {
      const type = parameter.childForFieldName("type");
      return type !== null && sourceText(type, file.current) === `${contextAlias}.Context`;
    }).map((parameter) => parameter.childForFieldName("name")).filter((name2) => name2 !== null).map((name2) => sourceText(name2, file.current))
  );
  if (contextNames.size === 0) return;
  for (const list of descendants(body2, "statement_list")) {
    const statements = list.namedChildren;
...
function contextAwareErrorAssignment(statement, source, contextNames) {
  if (statement.type !== "short_var_declaration" && statement.type !== "assignment_statement") return void 0;
  ...
  const values = right?.namedChildren ?? [];
  if (!names.includes("err") || values.length !== 1 || values[0].type !== "call_expression") return void 0;
  const argumentsNode = values[0].childForFieldName("arguments");
  if (argumentsNode === null) return void 0;
  const contextName = [...contextNames].find((name2) => containsIdentifier(argumentsNode, source, name2));
  return contextName === void 0 ? void 0 : { contextName, errorName: "err" };
}
function testsNonNil(statement, source, errorName) {
...
function hasOrdinaryErrorHandling(statement, source) {
  ...
  return descendants(statement, "call_expression").some((call) => {
    ...
    return /^(?:Error|Errorf|Errorln|Warn|Warnf|Warnln|Nack|Reject|Fail|Failf|Fatal|Fatalf)$/i.test(sourceText(field, source));
  });
}
function classifiesCancellation(statement, source, contextName, errorName, contextAlias) {
  ...
  return compact.includes(`.Is(${errorName},${contextAlias}.Canceled)`) || compact.includes(`.Is(${errorName},${contextAlias}.DeadlineExceeded)`);
}
function identifierUsedOutsideBinding(body2, source, name2, bindingLeft) {
  let used = false;
  walk2(body2, (candidate) => {
    if (used || candidate.type !== "identifier") return;
    if (sourceText(candidate, source) !== name2) return;
...
function analyzeChannels(file, body2, signals) {
  const statementLists = descendants(body2, "statement_list");
...
function analyzeLoopVarCapture(file, body2, signals) {
  for (const forStatement of descendants(body2, "for_statement")) {
    ...
    if (forBody === void 0 || forBody === null) continue;
    const statements = forBody.namedChildren.find((node) => node.type === "statement_list")?.namedChildren ?? forBody.namedChildren;
    for (let index = 0; index < statements.length; index += 1) {
      const statement = statements[index];
      if (statement.type !== "go_statement") continue;
      const call = statement.namedChildren.find((node) => node.type === "call_expression");
      const literal = call?.childForFieldName("function");
      if (literal === void 0 || literal === null || literal.type !== "func_literal") continue;
      const paramNames = new Set(
        descendants(literal.childForFieldName("parameters") ?? literal, "parameter_declaration").flatMap((parameter) => parameter.namedChildren.filter((node) => node.type === "identifier").map((node) => sourceText(node, file.current)))
      );
      const previous = index > 0 ? statements[index - 1] : void 0;
      const shadowed = previous === void 0 ? /* @__PURE__ */ new Set() : shadowingNames(previous, file.current, loopVars);
...
function loopVariableNames(forStatement, source) {
  ...
  if (rangeClause !== void 0) {
    ...
    for (const child of left?.namedChildren ?? []) {
      if (child.type === "identifier") {
        const name2 = sourceText(child, source);
        if (name2 !== "_") names.add(name2);
      }
    }
    return names;
  }
  ...
  if (forClause !== void 0) {
    const init2 = forClause.childForFieldName("initializer") ?? forClause.namedChildren[0];
    if (init2?.type === "short_var_declaration") {
      const left = init2.childForFieldName("left");
      for (const child of left?.namedChildren ?? []) {
        if (child.type === "identifier") {
          const name2 = sourceText(child, source);
          if (name2 !== "_") names.add(name2);
...
function shadowingNames(statement, source, loopVars) {
  ...
  for (let index = 0; index < names.length; index += 1) {
    const name2 = names[index];
    if (loopVars.has(name2) && values[index] === name2) shadowed.add(name2);
  }
  return shadowed;
}
function analyzeSelectDefaultBusy(file, body2, signals) {
  for (const forStatement of descendants(body2, "for_statement")) {
    const forBody = forStatement.namedChildren.find((node) => node.type === "block");
    if (forBody === void 0) continue;
    ...
    const cases = selectNode.namedChildren.filter((node) => node.type === "default_case" || node.type === "communication_case");
...
function analyzeTickers(file, fn, body2, aliases, signals) {
  const timeAlias = aliases.get("time");
  if (timeAlias === void 0) return;
  const functionName = functionNameOf(fn, file.current);
  for (const call of descendants(body2, "call_expression")) {
    const functionNode = call.childForFieldName("function");
    if (functionNode === null) continue;
  ...
  const tickerNames = /* @__PURE__ */ new Map();
  ...
  for (const [name2, node] of tickerNames) {
    if (selectorCalls(body2, file.current, name2, "Stop").length > 0) continue;
    signals.push(signal(
      file,
      node,
      "go-concurrency.ticker.not-stopped",
      `${timeAlias}.NewTicker result ${name2} is never Stopped in this function.`,
      { helper: "NewTicker", ticker: name2 }
    ));
  }
}
function analyzeTimers(file, body2, aliases, signals) {
  const timeAlias = aliases.get("time");
  if (timeAlias === void 0) return;
  for (const forStatement of descendants(body2, "for_statement")) {
...
function functionNameOf(fn, source) {
...
function unbufferedChannelDeclaration(statement, source) {
  if (statement.type !== "short_var_declaration") return void 0;
  const left = statement.childForFieldName("left");
  const right = statement.childForFieldName("right");
  const names = left?.namedChildren ?? [];
  const values = right?.namedChildren ?? [];
  if (names.length !== 1 || values.length !== 1 || values[0]?.type !== "call_expression") return void 0;
  const call = values[0];
  if (sourceText(call.childForFieldName("function") ?? call, source) !== "make") return void 0;
  const args2 = call.childForFieldName("arguments")?.namedChildren ?? [];
  if (args2[0]?.type !== "channel_type") return void 0;
  if (args2.length > 2 || args2.length === 2 && sourceText(args2[1], source) !== "0") return void 0;
  return { name: sourceText(names[0], source) };
}
function directBlockingOperation(statement, source, channel) {
  ...
  const receive = descendants(statement, "unary_expression").find((node) => {
    const operand = node.childForFieldName("operand");
    return operand !== null && sourceText(operand, source) === channel && sourceText(node, source).startsWith("<-") && !hasNestedFunctionAncestor(node, statement);
  });
  return receive === void 0 ? void 0 : { node: receive, kind: "receive" };
}
function hasNestedFunctionAncestor(node, boundary) {
  let current = node.parent;
  while (current !== null && current.id !== boundary.id) {
...
function selectorCalls(node, source, receiver, field) {
...
function callsIdentifier(node, source, name2) {
  return descendants(node, "call_expression").some((call) => {
    const fn = call.childForFieldName("function");
    return fn?.type === "identifier" && sourceText(fn, source) === name2;
  });
}
function containsIdentifier(node, source, name2) {
  let found = false;
  walk2(node, (candidate) => {
    if (candidate.type === "identifier" && sourceText(candidate, source) === name2) found = true;
  });
  return found;
}
...
function hasSerializationProofInFunc(fn, source) {
  const body2 = fn.childForFieldName("body");
  if (body2 === null) return false;
  const bodyText = sourceText(body2, source);
  const lower = bodyText.toLowerCase().replace(/\s+/g, "");
  const hasActiveAdd = /atomic\.(AddInt32|AddInt64)\s*\([^)]*active/.test(bodyText);
  const hasMaxCheck = /Add(Int32|Int64)?\s*\([^)]*active[^)]*\)\s*[>]=?\s*1/.test(bodyText) || />\s*1/.test(bodyText) && /Add.*active/.test(bodyText) || lower.includes("active>1") || lower.includes("active>=2");
  if (hasActiveAdd && hasMaxCheck) return true;
  const hasAssertWithKeyword = (() => {
    const calls = descendants(body2, "call_expression");
    for (const call of calls) {
      ...
      if ((fnText.includes("error") || fnText.includes("fatal") || fnText.includes("assert")) && (callText.includes("concurrent") || callText.includes("overlap") || callText.includes("serial"))) {
        return true;
      }
    }
    return false;
...
function analyzeMissingSerializationTest(file, root, signals) {
  const source = file.current;
  const LIFECYCLE = /* @__PURE__ */ new Set(["Export", "ForceFlush", "Shutdown", "OnEmit"]);
  const goStmts = descendants(root, "go_statement");
  const launchesByTest = /* @__PURE__ */ new Map();
...
function analyzeAsyncListenerMissingClose(file, root, signals) {
  ...
  for (const method of descendants(root, "method_declaration")) {
    ...
    const methodName2 = functionNameOf(method, file.current);
    if (typeName === void 0 || methodName2 === void 0) continue;
    if (methodName2 === "Close" || methodName2 === "Shutdown") {
...
function isGoCall(call) {
  let current = call.parent;
  while (current !== null) {
    if (current.type === "go_statement") return true;
    if (current.type === "func_literal" || current.type === "function_declaration" || current.type === "method_declaration") return false;
    current = current.parent;
  }
  return false;
}
...
function stableSignals(signals) {
  return signals.sort((left, right) => left.path.localeCompare(right.path) || left.line - right.line || left.ruleId.localeCompare(right.ruleId));
}
function stablePositives(positives) {
  const seen = /* @__PURE__ */ new Set();
  return positives.sort((left, right) => left.path.localeCompare(right.path) || left.line - right.line || left.key.localeCompare(right.key)).filter((positive) => {
...
async function discoverSources(ctx) {
  ...
  return {
    ...
    ...modulePath === void 0 ? {} : { modulePath },
    files
  };
}
function modulePathFromSources(sources) {
  const module2 = sources.find((source) => source.path === "go.mod");
  if (module2 === void 0) return void 0;
  return modulePathFromText(module2.content);
}
async function modulePathFromRepository(repoPath) {
  try {
    return modulePathFromText(await readFile4(join4(repoPath, "go.mod"), "utf8"));
  } catch {
    return void 0;
  }
}
function modulePathFromText(content) {
  return /^\s*module\s+([^\s/][^\s]*)\s*$/m.exec(content)?.[1];
}
async function changedSource(repoPath, change, path) {
  const base = change?.baseRef;
  if (base === void 0 || !await existsAtRevision(repoPath, base, path)) {
    return { changedLines: /* @__PURE__ */ new Set(), status: "added" };
  }
  const args2 = ["diff", "--unified=0", base];
  const head = change?.headRef;
  if (head !== void 0 && !change?.worktree) args2.push(head);
  args2.push("--", path);
  const patch = await gitOutput(repoPath, args2);
  const previous = await gitOutput(repoPath, ["show", `${base}:${path}`]);
...
var GO_CONCURRENCY_MODEL_PROMPT = `You are reviewing Go concurrency for lifecycle races, cancellation ownership, deadlocks, and unsafe channel or synchronization patterns.
...
var GO_CONCURRENCY_MODEL_SCHEMA = {
  type: "object",
  additionalProperties: false,
...
function prepareModelInputFromDiscovery(change, analysis, files) {
  ...
  ).slice(0, MAX_DETERMINISTIC_SIGNALS).map((signal2) => {
    const id = evidenceIdForSignal(signal2);
    evidenceCatalog.push({
      id,
      kind: "deterministic",
      path: signal2.path,
      line: signal2.line,
      message: signal2.message,
      snippet: signal2.snippet.slice(0, 300)
    });
    return {
      id,
      ruleId: signal2.ruleId,
      path: signal2.path,
      line: signal2.line,
      message: signal2.message,
      snippet: signal2.snippet.slice(0, 300)
  ...
  const pathOrder = [
    .../* @__PURE__ */ new Set([
      ...analysis.signals.map((signal2) => signal2.path),
      ...change?.changedFiles ?? [],
...
function buildModelReviewRequestFromDiscovery(change, analysis, files) {
  ...
  return {
    ...
    evidenceById,
    request: {
      prompt: GO_CONCURRENCY_MODEL_PROMPT,
      input,
      schema: GO_CONCURRENCY_MODEL_SCHEMA,
      budget: {
...
async function applyModelConcurrencyReview(ctx, output, evidenceById, staticSeverities = [], staticPrimaryConcern) {
  ...
  for (const observation of output.observations.slice(0, MAX_MODEL_OBSERVATIONS)) {
    ...
    ctx.review.observe({
      key: `go-concurrency.model.${observation.id}`,
...
async function runModelConcurrencyReview(ctx, analysis, files, staticSeverities = [], staticPrimaryConcern) {
  ...
  try {
    ...
    await applyModelConcurrencyReview(
      ctx,
      result.output,
      evidenceById,
      staticSeverities,
...
function evidenceIdForSignal(signal2) {
  return `det:${signal2.ruleId}:${signal2.path}:${signal2.line}`;
}
...
async function attachImportNavigation(ctx, analysis) {
  ...
  const seen = /* @__PURE__ */ new Set();
  for (const signal2 of analysis.signals) {
    const importers = await productionImporters(index, signal2.path);
    if (importers.length === 0) {
      continue;
    }
    const key = `navigation.importers:${signal2.path}:${signal2.ruleId}`;
    if (seen.has(key)) {
      continue;
    }
    seen.add(key);
    ctx.review.observe({
      key,
      summary: `${signal2.ruleId} in ${signal2.path} is imported by production package(s): ${importers.slice(0, 8).join(", ")}.`,
      metadata: {
        role: "navigation",
        source: "repo-index",
        signalPath: signal2.path,
        ruleId: signal2.ruleId,
        importers: importers.slice(0, 20)
      },
      evidence: [
        {
          location: { file: signal2.path, line: signal2.line },
          message: signal2.message,
...
// src/review.ts
async function reviewConcurrency(ctx, analysis, discoveryFiles = []) {
  ...
  emitGroupedFinding(ctx, timers, {
    ...
    impact: goVersionAtLeast(analysis.goVersion, 1, 23) ? "Each iteration allocates a timer, adding allocation and scheduling churn under tight loops." : "Timers can accumulate until their duration elapses, increasing memory use and timer-queue pressure.",
  ...
  emitGroupedFinding(ctx, asyncListenerMissingClose, {
    ...
    category: "reliability",
    severity: "high",
    summary: (count) => `${count} type${count === 1 ? "" : "s"} start a proven asynchronous Serve/ListenAndServe path and expose no Close or Shutdown mechanism.`,
    whyItMatters: "A direct goroutine launch or locally proven helper starts serving after Start returns. Without Close or Shutdown on the owning type, daemon teardown cannot stop that goroutine or the socket.",
...
function matching(analysis, ruleId) {
  return analysis.signals.filter((signal2) => signal2.ruleId === ruleId);
}
function timerSeverity(goVersion) {
  return goVersionAtLeast(goVersion, 1, 23) ? "low" : "medium";
}
function goVersionAtLeast(goVersion, major, minor) {
  if (goVersion === void 0) return false;
  return goVersion.major > major || goVersion.major === major && goVersion.minor >= minor;
}
function emitGroupedFinding(ctx, signals, input) {
  ...
  ctx.finding({
    ...
    whyItMatters: input.whyItMatters,
    impact: input.impact,
    evidence: signals.slice(0, 12).map((signal2) => ({
      location: {
        file: signal2.path,
        line: signal2.line,
        ...signal2.endLine === void 0 ? {} : { endLine: signal2.endLine }
      },
      message: signal2.message,
      snippet: signal2.snippet,
      data: signal2.data
    })),
...
function addPositives(ctx, analysis) {
  ...
  for (const [key, positives] of [...byKey].sort(([left], [right]) => left.localeCompare(right))) {
    const summary = key === "go-concurrency.waitgroup-ordered" ? positives.length === 1 ? "1 WaitGroup lifecycle registers work before launch and defers completion in the worker." : `${positives.length} WaitGroup lifecycles register work before launch and defer completion in the worker.` : positives.length === 1 ? "1 derived context lifecycle retains and invokes its cancellation function." : `${positives.length} derived context lifecycles retain and invoke their cancellation functions.`;
    ctx.review.positive({
      key,
      summary,
...
function addAssessment(ctx, groups) {
  ...
  if (groups.timers.length > 0) {
    ...
    ctx.review.assessment({
      risk: severity,
      summary: severity === "low" ? "time.After is used inside a loop; on Go 1.23+ this is mainly allocation churn rather than a hard leak." : "time.After is used inside a loop, which can accumulate unstopped timers until each fires."
    });
    ctx.review.opinion({
      ship: severity === "low",
      summary: severity === "low" ? "I would prefer NewTimer/Reset, but this is not a merge blocker on Go 1.23+." : "I would replace in-loop time.After with a reusable timer before merging."
    });
    return;
  }
...
function createApp() {
  ...
  app.rule("go-concurrency.review", async (ctx) => {
    ...
    ctx.review.observe({
      ...
      summary: analysis.mode === "diff" ? `Parsed ${analysis.filesScanned} changed Go files against ${analysis.base}.` : `Parsed ${analysis.filesScanned} Go files in repository review mode.`,
      metadata: {
