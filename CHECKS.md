...
| --- | --- | --- |
| `go-concurrency.async-listener.missing-close` | High | A type starts `Serve`/`ListenAndServe` in a direct goroutine, or passes a listener created by unshadowed standard-library `net.Listen` and its owned `*http.Server.Serve` callback at the exact parameter positions of a reachable, module-qualified local async helper, but has no reachable field-bound `Close`/`Shutdown`/`Stop` mechanism; unresolved, dead, shadowed, or reassigned relationships, synchronous helpers, process-lifetime `main`, tests, and helpers that directly stop the listener stay quiet |
