...
export const GO_CONCURRENCY_MODEL_PROMPT = `You are reviewing Go concurrency for lifecycle races, cancellation ownership, deadlocks, and unsafe channel or synchronization patterns.
...
export const GO_CONCURRENCY_MODEL_SCHEMA: Record<string, unknown> = {
  type: "object",
...
export interface ModelConcurrencyObservation {
  id: string;
  title: string;
  category: string;
...
export interface ModelConcurrencyReview {
  assessment: {
  ...
  observations: ModelConcurrencyObservation[];
}
...
export async function applyModelConcurrencyReview(
  ctx: RuleContext,
  output: ModelConcurrencyReview,
  evidenceById: Map<string, PreparedEvidenceItem>,
...
export async function runModelConcurrencyReview(
  ctx: RuleContext,
  analysis: Analysis,
  files: DiscoveryFile[],
  staticSeverities: StaticSeverity[] = [],
  ...
  try {
    const result = await ctx.model.review<ModelConcurrencyReview>(request);
...
function severityRank(severity: StaticSeverity | ModelConcurrencyObservation["severity"]): number {
  switch (severity) {
    case "critical":
      return 4;
    case "high":
...
function maxSeverity(values: Array<StaticSeverity | ModelConcurrencyObservation["severity"]>): StaticSeverity {
